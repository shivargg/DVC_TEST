    ...
    USE DATABASE DS_GLOC_DEV_DB;
    USE SCHEMA "00_REF";
    CREATE OR REPLACE PROCEDURE DS_GLOC_DEV_DB."00_REF".CONSTRUCT_00_REF_TABLES_TOP_AGENT()
    RETURNS VARCHAR
    LANGUAGE javascript
    EXECUTE AS CALLER
    AS
        ...
        $$
        var rs = 0;
        rs = snowflake.execute({sqlText: `CREATE OR REPLACE TABLE DS_GLOC_DEV_DB."00_REF"."00_BRANCH_AGENT_MAPPING" AS (
    SELECT
        "AgentId" AS AGT_ID,
...
rs = snowflake.execute({sqlText: `CREATE OR REPLACE TABLE DS_GLOC_DEV_DB."00_REF"."BRANCH_MANAGERS_V2" (
    UNIT_NUM VARCHAR,
    MANAGER_EMAIL VARCHAR
)`});
rs = snowflake.execute({sqlText: `INSERT INTO DS_GLOC_DEV_DB."00_REF"."BRANCH_MANAGERS_V2"
...
rs = snowflake.execute({sqlText: `CREATE OR REPLACE TABLE DS_GLOC_DEV_DB."00_REF"."00_MANAGER_IDS" AS (
    SELECT
...
rs = snowflake.execute({sqlText: `CREATE OR REPLACE TABLE DS_GLOC_DEV_DB."00_REF"."00_BRANCH_MANAGER_ASSOCIATIONS" AS (
